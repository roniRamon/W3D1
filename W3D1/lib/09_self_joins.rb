...
def craiglockhart_to_sighthill
  ...
  SELECT DISTINCT
   ...
   FROM
     routes a
   JOIN
     routes start ON (a.company = start.company AND a.num = start.num)
   JOIN
     stops transfer ON (start.stop_id = transfer.id)
   JOIN
    routes from_transfer ON (transfer.id = from_transfer.stop_id)
    JOIN
    routes destination ON (destination.company = from_transfer.company AND destination.num = from_transfer.num)
    WHERE a.stop_id = (
      SELECT id
      from stops
      WHERE name = 'Craiglockhart'
